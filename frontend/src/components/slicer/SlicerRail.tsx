      ...
      {presets && (
        <div className="flex flex-col gap-2">
          <PresetDropdown
            ...
            // Locked in embedded mode for the same reason the modal locks it
            // (#2611): the pick is unused on that path, and changing it away
            // from the design's target would drop canUseEmbedded and yank the
            // toggle out from under the user.
          ...
          {filamentSlotsLoading ? (
            ...
            <div className="flex flex-col gap-2" data-testid="filament-slots">
              {filamentSlots.map((slot, index) => {
                ...
                // auto-picked and disabled — the CLI still needs a profile per
                // project slot, but the user shouldn't have to think about
                // slots their plate doesn't paint with.
